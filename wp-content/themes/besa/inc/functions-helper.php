...
if (!function_exists('besa_tbay_tagregexp')) {
  function besa_tbay_tagregexp()
  {
    return apply_filters(
      'besa_tbay_custom_tagregexp',
      'video|audio|playlist|video-playlist|embed|besa_tbay_media'
    );
  }
}
          ...
          $theme = [
            $folder => [
              ...
              'img' =>
                ...
                '/inc/assets/images/active_theme/' .
                $folder .
                '.jpg',
            ],
          ];
...
if (!function_exists('besa_tbay_get_header_layouts')) {
  ...
  {
    $headers = ['header_default' => esc_html__('Default', 'besa')];
    $args = [
      'posts_per_page' => -1,
      'offset' => 0,
      'orderby' => 'date',
...
if (!function_exists('besa_tbay_get_footer_layouts')) {
  ...
  {
    $footers = ['footer_default' => esc_html__('Default', 'besa')];
    $args = [
      'posts_per_page' => -1,
      'offset' => 0,
      'orderby' => 'date',
  ...
  function besa_tbay_get_page_layout_configs()
  {
    ...
    if (isset($post->ID)) {
      ...
      switch (get_post_meta($post->ID, 'tbay_page_layout', true)) {
        case 'left-main':
          $configs['sidebar'] = ['id' => $left, 'class' => 'col-12 col-lg-3'];
          $configs['main'] = ['class' => 'col-12 col-lg-9'];
          break;
        case 'main-right':
          $configs['sidebar'] = ['id' => $right, 'class' => 'col-12 col-lg-3'];
          $configs['main'] = ['class' => 'col-12 col-lg-9'];
          break;
        case 'main':
          $configs['main'] = ['class' => 'col-12'];
          break;
        default:
          $configs['main'] = ['class' => 'col-12'];
          break;
  ...
  function besa_tbay_post_media($content)
  {
    ...
    if (!empty($media)) {
      ...
      $content = do_shortcode(
        $wp_embed->run_shortcode('[embed]' . $media . '[/embed]')
      );
    } else {
    ...
    if (!empty($media)) {
      ...
      $output .= $is_video
        ? '<div class="pro-fluid"><div class="pro-fluid-inner">'
        : '';
      $output .= $content;
...
if (!function_exists('besa_tbay_post_gallery')) {
  function besa_tbay_post_gallery($content)
  {
    ...
    preg_match('/' . $pattern . '/s', $content, $media);
    if (!empty($media[2])) {
      return '<div class="entry-gallery">' .
        do_shortcode($media[0]) .
        '<hr class="pro-clear" /></div>';
    }
...
if (!function_exists('besa_tbay_get_testimonials_layouts')) {
  function besa_tbay_get_testimonials_layouts()
  {
    $testimonials = [];
    $files = glob(
      get_template_directory() . '/vc_templates/testimonial/testimonial.php'
    );
    if (!empty($files)) {
...
if (!function_exists('besa_tbay_get_blog_layouts')) {
  function besa_tbay_get_blog_layouts()
  {
    ...
    $files = glob(
      get_template_directory() . '/vc_templates/post/carousel/_single_*.php'
    );
    if (!empty($files)) {
      foreach ($files as $file) {
        $str = str_replace(
...
if (!function_exists('besa_tbay_blog_image_sizes_full')) {
  function besa_tbay_blog_image_sizes_full()
  {
    ...
    if (!is_singular('post')) {
      if (
        isset($sidebar_configs['image_sizes']) &&
        $sidebar_configs['image_sizes'] == 'full'
      ):
        $style = true;
      endif;
    }
...
if (!function_exists('besa_tbay_share_js')) {
  ...
  {
    if (
      !besa_tbay_get_config('enable_code_share', false) ||
      besa_tbay_get_config('select_share_type') == 'custom'
    ) {
      return;
    }
    if (is_single()) {
...
if (!function_exists('besa_get_select_preloader')) {
  ...
  function besa_get_select_preloader()
  {
    ...
    if (isset($preloader)) {
      switch ($preloader) { case 'loader1': ?>
	                <div class="tbay-page-loader">
					  	<div id="loader"></div>
					  	<div class="loader-section section-left"></div>
					  	<div class="loader-section section-right"></div>
					</div>
	    			<?php break;case 'loader2': ?>
					<div class="tbay-page-loader">
	    			...
	    			<?php break;case 'loader6': ?>
					<div class="tbay-page-loader">
					    <div class="tbay-loader tbay-loader-six"> <span class=" spinner-cube-1 spinner-cube"></span> <span class=" spinner-cube-2 spinner-cube"></span> </div>
					</div>
	    			<?php break;case 'custom_image': ?>
					<div class="tbay-page-loader loader-img">
						<?php if (isset($media['url']) && !empty($media['url'])): ?>
					   		<img alt="<?php echo esc_attr($media['alt']); ?>" src="<?php echo esc_url(
  $media['url']
); ?>">
						<?php endif; ?>
					</div>
...
if (!function_exists('besa_tbay_archive_blog_size_image')) {
  function besa_tbay_archive_blog_size_image()
  {
    ...
    $blog_size = isset($_GET['blog_image_sizes'])
      ? $_GET['blog_image_sizes']
      : $blog_size;
...
if (!function_exists('besa_tbay_archive_image_position')) {
  function besa_tbay_archive_image_position()
  {
    ...
    $position = isset($_GET['image_position'])
      ? $_GET['image_position']
      : $position;
...
if (!function_exists('besa_tbay_autocomplete_search')) {
  function besa_tbay_autocomplete_search()
  {
    if (besa_tbay_get_global_config('autocomplete_search', true)) {
      add_action(
        'wp_ajax_besa_autocomplete_search',
        'besa_tbay_autocomplete_suggestions'
      );
      add_action(
        'wp_ajax_nopriv_besa_autocomplete_search',
        'besa_tbay_autocomplete_suggestions'
      );
    }
  }
...
if (!function_exists('besa_tbay_header_mobile_position')) {
  function besa_tbay_header_mobile_position()
  {
    ...
    $position = isset($_GET['header_mobile'])
      ? $_GET['header_mobile']
      : $position;
...
if (!function_exists('besa_tbay_the_topbar_mobile')) {
  ...
  {
    if (!besa_tbay_get_config('mobile_header', true)) {
      return;
    }
...
if (!function_exists('besa_tbay_custom_form_login')) {
  function besa_tbay_custom_form_login()
  {
    if (
      !(
        defined('BESA_WOOCOMMERCE_CATALOG_MODE_ACTIVED') &&
        BESA_WOOCOMMERCE_CATALOG_MODE_ACTIVED
      ) &&
      defined('BESA_WOOCOMMERCE_ACTIVED') &&
      BESA_WOOCOMMERCE_ACTIVED
    ) {
  ...
  function besa_tbay_get_variation_swatchs()
  {
    ...
    if (!empty($attribute_taxonomies)) {
      foreach ($attribute_taxonomies as $key => $tax) {
        $attribute_taxonomy_name = wc_attribute_taxonomy_name(
          $tax->attribute_name
        );
...
if (!function_exists('besa_tbay_get_title_mobile')) {
  function besa_tbay_get_title_mobile($title = '')
  {
    if (is_search()) {
      $title =
        esc_html__('Search results for', 'besa') .
        ' "' .
        get_search_query() .
        '"';
    } elseif (is_tag()) {
      $title =
        esc_html__('Posts tagged "', 'besa') .
      ...
      $title =
        esc_html__('Articles posted by ', 'besa') . $userdata->display_name;
    } elseif (is_404()) {
    ...
    if (class_exists('YITH_Woocompare')) { ?>
            ...
            <div class="yith-compare-header product">
                <a href="<?php echo esc_url(
                  $yith_woocompare->obj->view_table_url()
                ); ?>" class="compare added">
...
if (!function_exists('besa_tbay_check_data_responsive')) {
  function besa_tbay_check_data_responsive(
    $desktop,
    $desktopsmall,
    $tablet,
    $landscape_mobile,
...
if (!function_exists('besa_tbay_check_data_responsive_carousel')) {
  function besa_tbay_check_data_responsive_carousel(
    $columns,
    $desktop,
    $desktopsmall,
    $tablet,
    ...
    $data_responsive = besa_tbay_check_data_responsive(
      $desktop,
      $desktopsmall,
      $tablet,
      $landscape_mobile,
    ...
    $datas .=
      " data-desktopsmallslick=\"" . $data_responsive['desktopsmall'] . "\"";
...
if (!function_exists('besa_tbay_check_data_responsive_grid')) {
  function besa_tbay_check_data_responsive_grid(
    $columns,
    $desktop,
    $desktopsmall,
    $tablet,
    ...
    $datas .=
      " data-desktopsmall=\"" .
      esc_attr($data_responsive['desktopsmall']) .
      "\"";
    $datas .= " data-tablet=\"" . esc_attr($data_responsive['tablet']) . "\"";
    $datas .=
      " data-landscape=\"" . esc_attr($data_responsive['landscape']) . "\"";
...
if (!function_exists('besa_tbay_check_data_carousel')) {
  function besa_tbay_check_data_carousel(
    $rows,
    $nav_type,
    $pagi_type,
    $loop_type,
...
if (!function_exists('besa_tbay_data_carousel')) {
  function besa_tbay_data_carousel(
    $rows,
    $nav_type,
    $pagi_type,
    $loop_type,
    ...
    $data_array = besa_tbay_check_data_carousel(
      $rows,
      $nav_type,
      $pagi_type,
      $loop_type,
    ...
    $datas .=
      " data-pagination=\"" . esc_attr($data_array['pagination']) . "\"";
    $datas .= " data-loop=\"" . esc_attr($data_array['loop']) . "\"";
    $datas .= " data-auto=\"" . esc_attr($data_array['auto']) . "\"";

    if ($data_array['auto'] == 'yes') {
      $datas .=
        " data-autospeed=\"" . esc_attr($data_array['autospeed']) . "\"";
    }

    $datas .=
      " data-unslick=\"" . esc_attr($data_array['disable_mobile']) . "\"";
...
if (!function_exists('besa_tbay_search_in_options')) {
  function besa_tbay_search_in_options()
  {
    ...
    $get_search_in = isset($_GET['search_in_options'])
      ? $_GET['search_in_options']
      : $search_in;

    if (in_array($get_search_in, ['only_title', 'all'])) {
...
if (!function_exists('besa_get_template_product_grid')) {
  ...
  add_filter(
    'besa_get_template_product_grid',
    'besa_get_template_product_grid',
    10,
    1
  );
...
if (!function_exists('besa_get_template_product_vertical')) {
  ...
  add_filter(
    'besa_get_template_product_vertical',
    'besa_get_template_product_vertical',
    10,
    1
  );
...
if (!function_exists('besa_dont_update_theme')) {
  ...
  {
    if (
      0 !== strpos($url, 'https://api.wordpress.org/themes/update-check/1.1/')
    ) {
      return $r;
...
if (!function_exists('besa_elements_ready_countdown_timer')) {
  ...
  {
    $array = ['product-flash-sales', 'product-count-down'];
...
if (!function_exists('besa_localize_translate')) {
  function besa_localize_translate()
  {
    ...
    $config = [
      'quantity_minus' => apply_filters(
        'besa_quantity_minus',
        '<i class="tb-icon tb-icon-minus"></i>'
      ),
      'quantity_plus' => apply_filters(
        'besa_quantity_plus',
    ...
    if (defined('BESA_WOOCOMMERCE_ACTIVED') && BESA_WOOCOMMERCE_ACTIVED) {
      $position = wp_is_mobile()
        ? 'right'
        : apply_filters('besa_cart_position', 10, 2);
      $woo_mode = besa_tbay_woocommerce_get_display_mode();
      $quantity_mode = besa_woocommerce_quantity_mode_active();
      // loader gif
      $loader = apply_filters(
        'besa_quick_view_loader_gif',
        BESA_IMAGES . '/ajax-loader.gif'
      );

      $config['current_page'] = get_query_var('paged')
        ? get_query_var('paged')
        : 1;

      $config['popup_cart_icon'] = apply_filters(
        'besa_popup_cart_icon',
        '<i class="tb-icon tb-icon-cross"></i>',
        2
      );
      $config['popup_cart_noti'] = esc_html__(
        'was added to shopping cart.',
        'besa'
      );

      $config['cart_position'] = $position;
      $config['ajax_update_quantity'] = (bool) besa_tbay_get_config(
      ...
      $config['thumbnail_gallery_class_element'] =
        '.' . besa_get_thumbnail_gallery_item();
    }
