	...
	class YITH_WCWL_Install
	{
		...
		public function is_installed()
		{
			...
			$number_of_tables = $wpdb->query(
				$wpdb->prepare('SHOW TABLES LIKE %s', "{$this->_table_items}%")
			);
		...
		private function _update_300()
		{
			...
			foreach ($options as $option) {
				...
				if ('color_wishlist_table' != $option) {
					$background_hover = get_option(
						"{$base_option_name}_hover_background"
					);
					$color_hover = get_option(
						"{$base_option_name}_hover_color"
					);
					$border_hover = get_option(
						"{$base_option_name}_hover_border_color"
					);
				}
			...
			$options = [
				...
				'yith_wcwl_color_button_style_1_hover' => [
			...
			update_option(
				'yith_wcwl_add_to_wishlist_style',
				$destination_value
			);
			...
			update_option(
				'yith_wcwl_add_to_cart_rounded_corners_radius',
				$radius_value
			);
			update_option(
				'yith_wcwl_ask_an_estimate_rounded_corners_radius',
				$radius_value
			);
		}
		...
		private function _add_wishlists_table()
		{
			...
			if (
				!$this->is_installed() ||
				version_compare(
					get_option('yith_wcwl_db_version'),
					'3.0.0',
					'<'
				)
			) {
		...
		private function _add_pages()
		{
			wc_create_page(
				sanitize_title_with_dashes(
					_x('wishlist', 'page_slug', 'yith-woocommerce-wishlist')
				),
